...
export default function Home() {
  ...
  const addSelection = () => {
    ...
    const id = `manual-${Date.now()}`; const newPanel: Panel = { id, name: `Auswahl ${panels.length + 1}`, indices: [...selection].sort((a, b) => a - b), color: COLORS[panels.length % COLORS.length], enabled: true, transform: { rotation: 0, scale: 1, flipX: false, flipY: false } };
      ...
      <section className="workspace">
        ...
        <section className="stage-card">
          ...
          <div className={`viewport ${stageMode === '2d' ? 'viewport-2d' : ''}`}>
            {stageMode === '3d' ? <><MapCanvas points={points} panels={panels} selectionMode={selectionMode} onSelection={indices => { setSelection(indices); setMessage(`${fmt.format(indices.length)} LEDs im Rahmen markiert.`); }} camera={mapCamera} onCameraChange={setMapCamera} /><div className="axis-chip"><i className="x" /> X <i className="y" /> Y <i className="z" /> Z</div><div className="canvas-help">{selectionMode ? 'Rahmen ziehen, um LEDs zu markieren' : 'Ziehen: drehen · Scrollen: zoomen · Shift: auswählen'}</div></> : <><FixturePreview panel={active} points={points} ledSize={settings.ledSize} interactive onTransform={updatePanelTransform} /><div className="stage-2d-controls"><label><span>Rotation</span><input type="range" min="-180" max="180" step="0.1" value={active?.transform.rotation ?? 0} onChange={event => active && updatePanelTransform({ ...active.transform, rotation: Number(event.target.value) })} /><output>{active?.transform.rotation.toFixed(1) ?? '0.0'}°</output></label><div className="stage-snap-buttons"><button onClick={() => snapActive('horizontal')}>↔ Snap H</button><button onClick={() => snapActive('vertical')}>↕ Snap V</button><button className={active?.transform.flipX ? 'active' : ''} onClick={() => flipActive('horizontal')}>⇋ Flip H</button><button className={active?.transform.flipY ? 'active' : ''} onClick={() => flipActive('vertical')}>⇵ Flip V</button></div><label><span>Exportgröße</span><input type="range" min="0.25" max="4" step="0.01" value={active?.transform.scale ?? 1} onChange={event => active && updatePanelTransform({ ...active.transform, scale: Number(event.target.value) })} /><output>{Math.round((active?.transform.scale ?? 1) * 100)} %</output></label><button className="stage-reset" onClick={() => updatePanelTransform({ rotation: 0, scale: 1, flipX: false, flipY: false })}>Zurücksetzen</button></div><div className="canvas-help">Ziehen: drehen · Mausrad: Exportgröße</div></>}
          </div>
