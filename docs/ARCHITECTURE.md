...
- **Initial module geometry follows the 3D camera.** The same yaw/pitch projection used by the scan canvas supplies each module's relative centre. Logical row/column neighbours estimate its projected axes, rotation, width, and height. Dimensions never shrink below one MMFL cell per physical LED step, even when a panel appears edge-on. Because depth overlaps cannot exist in a single MMFL plane, rotated module bounds are separated minimally before the common grid is produced. Re-alignment is explicit so later manual edits are not overwritten silently.
